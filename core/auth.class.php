...
class Auth extends MongoDBAdapter {
	...
	// TODO: move $_COOKIE handling to UI.
	...
	public static function logout($all_sessions = false, $hash = NULL, $ip = NULL) {
		...
		if ($all_sessions) {
			...
			self::db()->user_sessions->remove(['uid' => $user->uid, 'hash' => $hash, 'ip' => $ip]);
		}
...
class User extends MongoDBAdapter {
	private $___fields = [];
...
class Group extends MongoDBAdapter {
